...
import de.lucavinci.bungeeban.events.BungeeKickEvent;
        ...
        if(sender.hasPermission(ConfigManager.cv("general.permissions.kick"))) {
            if(args.length >= 2) {
                ...
                if(BungeeCord.getInstance().getPlayer(playername) != null) {
                    ...
                    if(!BungeeCord.getInstance().getPlayer(playername).hasPermission(ConfigManager.cv("general.permissions.exempt.ban"))) {
                        BungeeKickEvent event = new BungeeKickEvent(playername, reason, sender.getName());
                        ...
                        if (!event.isCancelled()) {
                            ...
                            sender.sendMessage(BungeeBan.PREFIX + ConfigManager.txt("errors.internalerror").replace("%ERROR%", "The kick was cancelled by an event"));
                        }
                    } else {
                        sender.sendMessage(BungeeBan.PREFIX + ConfigManager.txt("errors.internalerror").replace("%ERROR%", "You can't kick this player"));
                    }
                } else {
